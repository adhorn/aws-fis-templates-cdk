...
export class SecGroupExperiments extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const importedSSMASecGroupRoleArn = cdk.Fn.importValue('SSMASecGroupRoleArn');
    const importedStopConditionArn = cdk.Fn.importValue('StopConditionArn');
    const importedSecGroupSSMADocName = cdk.Fn.importValue('SecGroupSSMADocName');
    ...
    const templateSecGroup = new fis.CfnExperimentTemplate(this, 'fis-template-inject-nacl-fault',
      {
        ...
        tags: {
          Name: 'Security Group ingress rule fault',
