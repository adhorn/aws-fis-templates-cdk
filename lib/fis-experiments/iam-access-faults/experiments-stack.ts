...
export class IamAccessExperiments extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const importedSSMAIamAccessRoleArn = cdk.Fn.importValue('SSMAIamAccessRoleArn');
    const importedStopConditionArn = cdk.Fn.importValue('StopConditionArn');
    const importedIamAccessSSMADocName = cdk.Fn.importValue('IamAccessSSMADocName');
    ...
    const s3FaultPolicy = new iam.ManagedPolicy(this, 's3-deny', {
      statements: [
        new iam.PolicyStatement({
          ...
          resources: [
            `arn:aws:s3:::${importedS3BucketToDeny.toString()}`
          ],
          actions: [
            's3:PutObject',
            's3:GetObject',
    ...
    const startAutomation = {
      ...
      parameters: {
        documentArn: `arn:aws:ssm:${this.region}:${this.account}:document/${importedIamAccessSSMADocName.toString()}`,
    ...
    // Experiments
    const templateInjectS3AccessDenied = new fis.CfnExperimentTemplate(this, 'fis-template-inject-s3-access-denied',
      {
        description: 'Deny Access to a S3 to an IAM role',
        ...
        tags: {
          Name: 'S3Bucket-InjectAccessDenied',
